...
If you add new npm packages or edit the `package.json` files in any other way, you have to stop the development container and delete the images `typic-lists-backend` and `typic-lists-frontend`, and the volumes `typic-lists_node_modules_backend` and `typic-lists_node_modules_frontend` in Docker. If you want to reset the database, then stop the development container and delete the volume `typic-lists_database` in Docker.
